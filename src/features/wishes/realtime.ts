...
export function useRealtimeWallEvents(
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`public:realtime_wall_events:event_id=eq.${eventId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          setState(prev => {
            ...
            if (prev.status === 'reconnecting' || prev.status === 'disconnected') {
              onReconnect()
            }
            return { status: 'connected', lastSyncAt: new Date() }
          })
    ...
    const handleOnline = () => setState(prev => ({ ...prev, status: 'reconnecting' }))
    const handleOffline = () => setState(prev => ({ ...prev, status: 'disconnected' }))
