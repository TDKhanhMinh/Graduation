  ...
  return (
    <section className="space-y-4 rounded-3xl border border-border/80 bg-card p-4 shadow-sm sm:p-5" aria-labelledby="moderation-media-heading">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Quản lý tệp đa phương tiện</p>
          <h2 id="moderation-media-heading" className="mt-1 font-heading text-xl font-semibold">Tệp đa phương tiện trong hàng đợi</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">Chỉ hiển thị tệp đa phương tiện đã gắn với lời chúc trong trang hiện tại. Việc tải lên và xóa vẫn bị khóa khi chưa có hợp đồng an toàn.</p>
        </div>
        <div className="flex rounded-xl border bg-background p-1" role="group" aria-label="Lọc tệp đa phương tiện">
          {(["all", "image", "audio"] as const).map((value) => <Button key={value} type="button" size="sm" variant={filter === value ? "default" : "ghost"} onClick={() => { setFilter(value); setSelectedId(null) }} aria-pressed={filter === value}>{value === "all" ? "Tất cả" : value === "image" ? "Ảnh" : "Âm thanh"}</Button>)}
        </div>
      </div>
      ...
      {visibleMedia.length === 0 ? (
        <div className="flex min-h-32 items-center justify-center rounded-2xl border border-dashed bg-surface-sunken px-5 py-8 text-center">
          <div><Info className="mx-auto size-6 text-muted-foreground" /><p className="mt-2 text-sm font-medium">Chưa có tệp đa phương tiện phù hợp</p><p className="mt-1 text-xs text-muted-foreground">Tệp đa phương tiện không khả dụng hoặc chưa có trong trang hàng đợi sẽ không được dựng bản xem trước giả.</p></div>
        </div>
      ) : (
