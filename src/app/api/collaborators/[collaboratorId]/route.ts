...
async function getOwner(body: unknown) {
  ...
  if (!access) return { error: response({ error: 'Event not found' }, 404) }
...
export async function PATCH(request: Request) {
  ...
  const owner = await getOwner(body)
  ...
  const { data, error } = await createAdminClient().rpc('set_event_collaborator_role', {
    ...
    p_owner_id: owner.access.userId,
