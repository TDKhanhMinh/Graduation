...
export default async function PosterStudioPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fallbackDocument = createPosterDocumentFromQuickCreate({ eventId: event.id, eventCategory: category, templateId: "graduation-glow-01", ratio: "4:5", title: event.title, tagline: "Một khoảnh khắc đáng nhớ", date: event.event_date?.slice(0, 10) ?? "", location: "Sảnh Memoria", publicUrl, accent: "#c85b45" })
